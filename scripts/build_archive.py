...
:root{{
  ...
  /* --accent/--accent-ink/--accent-soft desaturated 2026-09-15 (was orange-terracotta
     #D97757/#F0A382/#3D2A21) to a neutral warm stone, matching the live claude.ai pages'
     same-day change - the old orange competed visually with the map's Milestone-orange
     news colour. --hot/--hot-soft were explicitly left unchanged (kept as the page's
     "important number" emphasis colour, distinct from UI chrome) - don't touch those. */
  --accent:#9C9890; --accent-ink:#D8D4C8; --accent-soft:#38362F;
...
/* Colour-swatch legend row under "Ranked by technical materiality" (added 2026-09-15,
   live pages only so far - kept here so an archived edition renders it correctly the
   day the daily pipeline starts emitting it too, instead of drifting silently). */
.legend{{display:flex;flex-wrap:wrap;gap:6px 16px;margin-top:0;padding-top:0;
...
/* g-assay-fresh/g-assay/g-cps kept byte-for-byte in sync with the live claude.ai pages'
   own rule (added 2026-09-15, .g-assay recoloured gold->sky-blue 2026-09-15 same day -
   gold read too close to the Milestone/Other-News orange+yellow pair) - an archived
   edition needs to render exactly as it looked live, not fall back to unstyled text (or
   a stale colour) because this template's copy had drifted behind. Green = the item's
   own freshly reported number, sky-blue = a quoted/historical comparison figure, purple
   = a raw counts-per-second reading, not a grade. */
.g-assay-fresh{{font-family:"IBM Plex Mono",monospace;font-weight:700;color:#00e64d;
  text-shadow:0 0 10px rgba(0,230,77,.45);letter-spacing:.01em;word-spacing:-.3em}}
.g-assay{{font-family:"IBM Plex Mono",monospace;font-weight:700;color:#38BDF8;
  text-shadow:0 0 10px rgba(56,189,248,.45);letter-spacing:.01em;word-spacing:-.3em}}
.g-cps{{font-family:"IBM Plex Mono",monospace;font-style:italic;font-weight:500;
  color:#B295DC;word-spacing:-.3em}}
